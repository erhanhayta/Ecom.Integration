...
/** UI'ya isim, API'ye sayı gitsin */
const FIRMS = [
    { id: 1, name: "Trendyol" },
    { id: 2, name: "Hepsiburada" },
    { id: 3, name: "N11" },
];

const firmName = (id) => FIRMS.find(f => f.id === Number(id))?.name || `Firm ${id}`;
...
function ShopForm({ initial, onSubmit, submitting }) {
    ...
    const change = (k) => (e) => {
        ...
        const isNumber = e.target.type === "number" || k === "firm";
    ...
    function submit(e) {
        e.preventDefault();
        onSubmit(form); // firm sayı olarak gider
    }
    ...
    return (
        <form onSubmit={submit} className="grid grid-cols-2 gap-3">
            ...
            <div>
                <label className="block text-sm text-gray-600 mb-1">Pazar Yeri (Firma)</label>
                <select className="w-full rounded-lg border px-3 py-2" value={form.firm} onChange={change("firm")}>
...
export default function Shops() {
    ...
    const filtered = useMemo(() => {
        ...
        return rows.filter(r =>
            ...
            firmName(r.firm).toLowerCase().includes(t) // isimden de ara
        );
    }, [rows, q]);
        ...
        <WindsterLayout>
            <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                    <input
                        placeholder="Ara (shopName / accountId / refCode / firm adı)"
                    ...
                    <table className="min-w-full text-sm">
                        <thead className="bg-gray-50 text-left">
                            <tr>
                                ...
                                <th className="px-4 py-3 w-32">Pazar Yeri (Firma)</th>
                        ...
                        <tbody className="divide-y">
                            ...
                            {!loading && !err && filtered.map(r => (
                                <tr key={r.id} className="hover:bg-gray-50 group">
                                    ...
                                    <td className="px-4 py-3">{firmName(r.firm)}</td>
